...
Tools and instructions for importing custom models into Honkai Impact.

#### DISCLAIMER: I strongly recommend only using private servers for modding. Using these tools on official Honkai servers carries a high risk of being banned. I do not condone the use of these tools and programs on official servers, and take no responsibility for any consequences as a result if you do. 

GIMI is a version of 3DMigoto that I have modified to be compatible with Honkai.
...
Model files for the importer are located at [HI-Model-Importer-Assets](https://github.com/SilentNightSound/HI-Model-Importer-Assets)

## Installation Instructions (3DMigoto)

1. Download a 3dmigoto .zip from [releases](https://github.com/SilentNightSound/HI-Model-Importer/releases) and extract it. I have provided two versions:
...
2. (for V6 GIMI and below; v7 and above does this automatically and you can skip this step) Depending on the location of your Honkai Impact .exe file, you may need to change this line in the d3dx.ini file to point to your own installation (the Honkai game .exe, not the Honkai launcher .exe - the one you want is usually located in the Honkai Impact Game folder). If changing it to your path doesn't work, try using `target = HonkaiImpact.exe`:

<img src="https://user-images.githubusercontent.com/107697535/174322200-b1afea95-53f5-4add-be89-698f85503908.png" width="800"/>

3. Double click "3DMigoto Loader.exe" to start the loader, then start up Honkai through the HonkaiImpact.exe. If everything is correct so far, 3DMigoto should be injected into the game and you should see a green text overlay (only if using the "for development" version, the "for playing" version does not show the green text):
...
2. Download and install the modified 3DMigoto plugin (blender_3dmigoto_gimi.py) from [releases](https://github.com/SilentNightSound/HI-Model-Importer/releases). You can install an add-on in Blender by going to Edit -> Preferences -> Add-Ons -> Install, then selecting the .py file. 
...
Also, if you any questions about modding come join the Honkai modding discord at https://discord.gg/gR2Ts6ApP7. The only verification is that you can 3dmigoto GIMI working by following the steps above.
